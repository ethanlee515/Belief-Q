  ...
  SimConfig.compile {
    ...
    for(t <- 0 until 100) {
      ...
      if(state == State.start_computing_cToV) {
        ...
        val var_label = Variable3D(0, DataError2D(2, 1))
        val variable = dut.graph.variables(var_label)
        println(f"prior($var_label) = ${variable.prior.toBigDecimal}")
        /*
        val initial_prior = dut.cached_inputs.initial_priors(var_label)
        println(f"cached prior($var_label) = ${initial_prior.toBigDecimal}")
        */
      }
